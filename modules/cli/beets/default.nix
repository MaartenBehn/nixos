{
  flake.modules.homeManager.server-cli = { lib, pkgs, ... }:
    let
      beets-plugin-xtractor = pkgs.callPackage ./_beets-plugin-xtractor.nix {
        inherit lib;
        inherit (pkgs) fetchPypi essentia-extractor python3Packages;
        beets = pkgs.python3Packages.beets-minimal;
      };

      beets-plugin-lidarrfields = pkgs.callPackage ./_beets-plugin-lidarrfields.nix {
        inherit lib;
        inherit (pkgs) fetchPypi python3Packages;
        beets = pkgs.python3Packages.beets-minimal;
      };

      beets-plugin-mood-writer = pkgs.callPackage ./_beets-plugin-mood-writer.nix {
        inherit lib pkgs;
        inherit (pkgs) python3Packages;
        beets = pkgs.python3Packages.beets-minimal;
      };

      beets-plugin-check = pkgs.callPackage ./_beets-plugin-check.nix {
        inherit lib;
        inherit (pkgs) fetchPypi python3Packages;
        beets = pkgs.python3Packages.beets-minimal;
      };
